<template>
  <div>
    <v-snackbar :color="currentNotificationColor" :timeout="-1" v-model="show">
      <v-progress-circular
        class="ma-2"
        indeterminate
        v-show="showProgress"
...
export default class NotificationsManager extends Vue {
  ...
  public async setNotification(notification: AppNotification | false) {
    ...
    if (notification) {
      ...
      this.indefinite = notification.indefinite || false;
      this.show = true;
    } else {
  ...
  public async onNotificationChange(
    ...
    if (newNotification !== this.currentNotification) {
      ...
      if (newNotification && !newNotification.indefinite) {
  ...
  public get currentNotificationTimeout() {
    if (this.currentNotification && this.currentNotification.indefinite) {
      return 0;
    } else {
      return 6500;
    }
